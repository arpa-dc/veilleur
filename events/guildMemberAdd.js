...
module.exports = {
  ...
  async execute(member, client) {
    // ─── ANTIBOT (ajout de bot non autorisé) ──────────────────────────────────
    if (member.user.bot) {
    ...
    if (raidDetected || (isLockdownActive() && newAccount)) {
      ...
      // En lockdown, ou pendant un raid avéré : on expulse directement les comptes très récents
      ...
      if (raidDetected && logsChannel) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`${joinCount} arrivées détectées en moins de 10 secondes (seuil : ${JOIN_THRESHOLD}). Pense à utiliser \`*lockdown\` si besoin.`)
          .setTimestamp();
        await logsChannel.send({ embeds: [embed] }).catch(() => {});
      }
    }
